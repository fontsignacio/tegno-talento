...
exports.validateCreateVacante = void 0;
const validateCreateVacante = (req, res, next) => {
    const { descripcion, fecha_cierre, puesto_id, peso_experiencia, peso_educacion, experiencia_req, nivel_educ_req, puntaje_corte, vacante_habilidades, } = req.body;
    // Validar campos obligatorios de la vacante
    if (!puesto_id)
        return res.status(400).json({ error: "El puesto_id es obligatorio" });
    if (peso_experiencia == null)
        return res.status(400).json({ error: "peso_experiencia es obligatorio" });
    if (peso_educacion == null)
        return res.status(400).json({ error: "peso_educacion es obligatorio" });
    if (experiencia_req == null)
        return res.status(400).json({ error: "experiencia_req es obligatorio" });
    if (nivel_educ_req == null)
        return res.status(400).json({ error: "nivel_educ_req es obligatorio" });
    if (puntaje_corte == null)
        return res.status(400).json({ error: "puntaje_corte es obligatorio" });
    // Validar habilidades
...
exports.validateCreateVacante = validateCreateVacante;
